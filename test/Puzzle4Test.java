...
class Puzzle4Test {
    ...
    @ParameterizedTest
    @CsvSource(value = {"aa bb cc dd ee:true", "aa bb cc dd aa:false", "aa bb cc dd aaa:true"}, delimiter = ':')
    void solve(String input, boolean valid) {
        assertEquals(valid, Puzzle4.solve(new String[]{input}) > 0L);
    }
    ...
    @ParameterizedTest
    @CsvSource(value = {"abcde fghij:true", "abcde xyz ecdab:false", "a ab abc abd abf abj:true",
            "iiii oiii ooii oooi oooo:true", "oiii ioii iioi iiio:false"}, delimiter = ':')
    void solveA(String input, boolean valid) {
        assertEquals(valid, Puzzle4.solveA(new String[]{input}) > 0L);
    }
